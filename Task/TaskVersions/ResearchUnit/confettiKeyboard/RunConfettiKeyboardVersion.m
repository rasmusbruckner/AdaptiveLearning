function allTaskData = RunConfettiKeyboardVersion(config, unitTest, cBal)
%RUNCONFETTIKEYBOARDVERSION This function runs the keyboard version of the
%confetti-cannon task.
%
%   Input
...
%
%   Testing
%       Integration test in progress: Related integration test -> "al_HamburgIntegrationTest"
...
if scanner == false
    concentration = [16, 8];
elseif scanner == true
    concentration = 12; % todo: check what we need for Jena
end
...
% Confetti end point
confettiEndMean = 100; % this is added to the circle radius
confettiEndStd = 4; % this is the spread around the average end point
...
trialflow.confetti = 'show confetti cloud';
trialflow.shieldAppearance = 'full';
...
if gParam.askSubjInfo == false || unitTest.run
    ...
    elseif scanner == true
        ...
        subject.checkGender();
        subject.checkGroup();
        % Todo Jena: add check for startsWithBlock (run) for scanner
    end
end
...
if scanner == false
    ...
    [allTaskData, totWin] = al_confettiKeyboardConditions(taskParam);

elseif scanner == true
    
    % Todo this has to be adjusted for Jena when we work with scanner 
