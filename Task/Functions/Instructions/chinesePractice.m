...
while 1
    ...
    switch(screenIndex)
        ...
        case 2
            ...
            %taskData = al_generateOutcomes(taskParam, taskParam.gParam.haz(1), taskParam.gParam.concentration(3), condition);
            taskData = al_generateOutcomesChinese(taskParam, taskParam.gParam.haz(1), taskParam.gParam.concentration(3), condition);
            % ---
            [taskData, ~] = al_mainLoop(taskParam, taskParam.gParam.haz(1), taskParam.gParam.concentration(3), condition, subject, taskData, trial);
            screenIndex = screenIndex + 1;
            WaitSecs(0.1);
        ...
        case 4
            ...
            taskParam.gParam.nContexts = 1;
            taskParam.gParam.nStates = 2;
            %[taskData, trial] = al_loadTaskData(taskParam, condition, taskParam.gParam.haz(1), taskParam.gParam.concentration(1));
            % --- 
            trial = taskParam.gParam.chinesePractTrials;
            taskData = al_generateOutcomesChinese(taskParam, taskParam.gParam.haz(1), taskParam.gParam.concentration(1), condition);
             % --- 
            [taskData, ~] = al_mainLoop(taskParam, taskParam.gParam.haz(1), taskParam.gParam.concentration(1), condition, subject, taskData, trial);
            sumCannonDev = sum(abs(taskData.cannonDev) >= 10);
            screenIndex = screenIndex + 1;
            WaitSecs(0.1);
