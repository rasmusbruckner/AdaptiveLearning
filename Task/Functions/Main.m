...
elseif taskParam.unitTest
    ...
    taskData.actJitter = nan(trial,1);
    taskData.block = ones(trial,1);
    taskData.pred = [180,20,30,360,50,20,90,10,40,180,360,250,30,80,280,180,180,10,180,180,]% ones(trial,1);
else
...
Data = struct(fieldNames.actJitter, taskData.actJitter, fieldNames.block,...
    taskData.block, fieldNames.initiationRTs, taskData.initiationRTs,...
    fieldNames.timestampOnset, taskData.timestampOnset,...
    fieldNames.timestampPrediction, taskData.timestampPrediction,...
    fieldNames.timestampOffset, taskData.timestampOffset,...
    fieldNames.allASS, taskData.allASS, fieldNames.driftConc, driftConc,...
    fieldNames.oddballProb, oddballProb, fieldNames.oddBall,...
    taskData.oddBall, fieldNames.ID, {taskData.ID}, fieldNames.age,...
    taskData.age, fieldNames.rew, {taskData.rew}, fieldNames.actRew,...
    taskData.actRew, fieldNames.sex, {taskData.sex}, fieldNames.cond,...
    {taskData.cond}, fieldNames.cBal, {taskData.cBal}, fieldNames.trial,...
    taskData.trial, fieldNames.vola, vola, taskParam.fieldNames.sigma,...
    sigma, fieldNames.outcome, taskData.outcome, fieldNames.distMean,...
    taskData.distMean, fieldNames.cp, taskData.cp, fieldNames.TAC,...
    taskData.TAC, fieldNames.boatType, taskData.boatType,...
    fieldNames.catchTrial, taskData.catchTrial, fieldNames.predT,...
    taskData.predT, fieldNames.outT, taskData.outT, fieldNames.triggers,...
    taskData.triggers, fieldNames.pred, taskData.pred,...
    fieldNames.predErr, taskData.predErr, fieldNames.predErrNorm,...
    taskData.predErrNorm, fieldNames.predErrPlus, taskData.predErrPlus,...
    fieldNames.predErrMin, taskData.predErrMin, fieldNames.memErr,...
    taskData.memErr, fieldNames.memErrNorm, taskData.memErrNorm,...
    fieldNames.memErrPlus, taskData.memErrPlus, fieldNames.memErrMin,...
    taskData.memErrMin, fieldNames.UP, taskData.UP,fieldNames.UPNorm,...
    taskData.UPNorm, fieldNames.UPPlus, taskData.UPPlus,...
    fieldNames.UPMin, taskData.UPMin, fieldNames.hit, taskData.hit,...
    fieldNames.perf, taskData.perf, fieldNames.accPerf, taskData.accPerf,...
    fieldNames.Date, {taskData.Date});
