classdef al_conditions
    ...
    % NOTE: This class is deprecated and will be deleted when all version
    % are independent
    ...
    methods
        ...
        function condobj = MainCondition(condobj, taskParam, subject)
            %MAINCONDITION   Runs the changepoint condition of the cannon task
            %
            %   Input
            %       condobj: task object with initialized values
            ...
            if condobj.runIntro && ~condobj.unitTest
                ...
                switch condobj.taskType
                    ...
                    case 'dresden'
                        ...
                        % Run main task
                        al_mainLoop(taskParam, condobj.haz(3), condobj.concentration(1), 'mainPractice', subject);
                        feedback = false;
            ...
            % [~, condobj.DataMain] = al_mainLoop(taskParam, condobj.haz(1), currentConcentration, 'main', subject, taskData, taskParam.gParam.trials);
        ...
        function condobj = FollowOutcomeCondition(condobj, taskParam, subject)
            ...
            if condobj.unitTest
                ...
                trial = taskParam.gParam.trials;
                taskData = al_generateOutcomesMain(taskParam, condobj.haz(1), currentConcentration, 'followOutcome');
                %              taskData.trial = nan;
            end

            currentConcentration = condobj.concentration(1);

            [~, condobj.DataFollowOutcome] = al_mainLoop(taskParam, condobj.haz(1), currentConcentration, 'followOutcome', subject, taskData, trial);
        ...
        function condobj = FollowCannonCondition(condobj, taskParam, subject)
            ...
            if condobj.unitTest
                ...
                trial = taskParam.gParam.trials;
                taskData = al_generateOutcomesMain(taskParam, condobj.haz(1), currentConcentration, 'followCannon');
                %              taskData.trial = nan;

            end
            currentConcentration = condobj.concentration(1);

            [~, condobj.DataFollowCannon] = al_mainLoop(taskParam, condobj.haz(1), currentConcentration, 'followCannon', subject, taskData, trial);
        ...
        function condobj = ReversalCondition(condobj, taskParam, subject)
            ...
            if condobj.runIntro && ~condobj.unitTest
                ...
                txtStartTask = ['This is the beginning of the task. During this block you will earn real money for your '...
                    'performance. The trials will be exactly the same as those in the in the practice session.\n\nOn each '...
                    'trial a cannon will aim at a location on the circle. The cannon will fire a ball somewhere near the point '...
                    'of aim. Most of the time the cannon will remain aimed at the same location, but occasionally the cannon '...
            ...
            if condobj.runIntro && ~condobj.unitTest
                ...
                elseif isequal(subject.session, '2') || isequal(subject.session, '3')
                    ...
                    txtStartTask = ['This is the beginning of the ODDBALL TASK. During this block you will earn real money '...
                        'for your performance. The trials will be exactly the same as those in the in the last session.\n\n'...
                        'On each trial a cannon will aim at a location on the circle. On most trials the cannon will fire a'...
                        'ball somewhere near the point of aim. However, on a few trials a ball will be shot '...
